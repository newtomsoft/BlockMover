...
public class SingleBlockMoveTest
{
    ...
    {
        var block = new BlockNew(2, Orientation.Horizontal, Coordinate.From(1, 0));
        var grid = new GridNew(new GridSize(3, 3), Coordinate.From(2, 2), new List<BlockNew> { block });
    ...
    {
        var block = new BlockNew(2, Orientation.Horizontal, Coordinate.From(0, 0));
        var grid = new GridNew(new GridSize(3, 3), Coordinate.From(2, 2), new List<BlockNew> { block });
    ...
    {
        var block = new BlockNew(2, Orientation.Vertical, Coordinate.From(0, 1));
        var grid = new GridNew(new GridSize(3, 3), Coordinate.From(2, 2), new List<BlockNew> { block });
        grid.CanBlockMove(0, Direction.Decrease).ShouldBeTrue();
        block.Decrease();
        block.Coordinate.ShouldBe(Coordinate.From(0, 0));
    ...
    {
        var block = new BlockNew(2, Orientation.Vertical, Coordinate.From(0, 0));
        var grid = new GridNew(new GridSize(3, 3), Coordinate.From(2, 2), new List<BlockNew> { block });
        grid.CanBlockMove(0, Direction.Decrease).ShouldBeFalse();
    }
